...
3. Затем с помощью <b>ikev2 authorization policy</b> необходимо описать те подсети к которым мы разрешим доступ из других филиалов.

4. Собираем IKEv2 Profile, при этом необходимо учесть следующее: требуется добавить проверку идентификатора <b>identity</b> для удалённого Spoke. В качестве альтернативы, как и на Hub(е), можно ограничиться проверкой только доменного имени (указание полного FQDN позволяет более точно разграничить взаимодействие между Spoke маршрутизаторами напрямую):
```
crypto ikev2 profile FLEXVPN-GerenalProfile-IKEV2
 description ==[ Profile of available authentication methods ]===
